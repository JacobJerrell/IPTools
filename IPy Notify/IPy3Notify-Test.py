...
# Find out if we need external or internal IP
IPTYPE = input("Enter I for Internal IP. Enter E for External IP. ")
...
def start():
    ...
    def getIP(name, path, kind):
        ...
        if kind == 'I':
            ...
            import json
            IP = json.loads(urllib.request.urlopen("http://ip.jsontest.com/").read())
            print('Current IP: ' + str(IP["ip"]))
            open(path, 'w').write(IP["ip"]) # Save the current IP address to the .dat file

        out(name, PATH_DAT, IPTYPE)
